...
{
  bool useV6 = false;
  bool m_dupAckSpoof = false;
  unsigned m_dcount = 10;

  CommandLine cmd;
  cmd.AddValue ("useIpv6", "Use Ipv6", useV6);
  cmd.AddValue ("dupAckSpoof", "Enable dupAck Spoofing", m_dupAckSpoof);
  cmd.AddValue ("dcount", "Number of DupAcks to be sent", m_dcount);
  cmd.Parse (argc, argv);

  NodeContainer nodes;
  nodes.Create (2);
  Config::SetDefault ("ns3::TcpSocketBase::DupAckSpoof", BooleanValue(m_dupAckSpoof));
  Config::SetDefault ("ns3::TcpSocketBase::dcount", UintegerValue(m_dcount));
  Config::SetDefault ("ns3::TcpSocketBase::Sack", BooleanValue(false));

  if (m_dupAckSpoof)
  {
  fp = fopen("SpoofEnabled", "w");
  }
  else
  {
  fp = fopen("SpoofDisabled", "w");
  }
